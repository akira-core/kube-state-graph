...
kubegraph.ParseStorageValues  ── StorageRequest{Start, End, Scope, Selector}
                                  ...
                                  pod-only roots with no ?namespace= derive the namespace selector
                                  (deriveStorageNamespaces — output-preserving, narrows the upstream only);
                                  an application= root suppresses that derivation, as a storage-side or node= root does
   ▼
Builder.BuildStorage(…, roots) ── readTopology under storagePlan: never issues kube_pod_container_info or the
                                  ...
                                  read BY REFERENCE in three more waves. An application= root first runs a
                                  recovery wave (appscope.go) in three stages — six controller-annotation
                                  families by tracking-id prefix, then kube_replicaset_owner / kube_job_owner
                                  by owner name, then kube_pod_owner once per kind — and readScopedPods waits
                                  on the claim-binding leg AND that recovery (and on
                                  kube_persistentvolumeclaim_annotations). kube_pod_info + kube_pod_owner are
                                  scoped to binding pods ∪ pod= root names ∪ recovered names, and under an
                                  application root the binding half narrows to related claims (podscope.go);
                                  the 4 kube_node_* families scoped to those pods' nodes ∪
                                  ...
                                  controller kind present and a matched volume). application= adds the
                                  recovery on top: 6 when nothing matches (and no pod query if the only
                                  bindings are unrelated); 6+1+2 then pods 2, nodes 4, controllers 3 for one
                                  Deployment- or CronJob-managed claimless pod; 6+2+6 on top of the 38-leg
                                  ...
                                  filtered to the loaded (az, env, cluster, ns, claim) keys; every query
                                  keeps the request's matchers and az routing — hub mode never reads
                                  another zone's store
   ▼
  ...
  **stock** Harvest `volume` label (the ONTAP FlexVol name). It is NOT a label
  ...
  - **hop A `volume_labels`** — the SOLE source of storage *topology*: the
    ...
    **A restricted read IS hub mode** (`topologyPlan.hub`, `claimscope.go`): the
    ...
    NOT reintroduce an az/env-relaxed or per-family-zone-routed hub read. Alert
    matching also AGREES ON ZONE (D11, `pkg/build/alerts.go`), for the builds
    that do hold several zones (unfiltered `/v1/graph`, catch-all backends): an alert's `az`/`env` pair must agree with the
...
The first external consumer is
