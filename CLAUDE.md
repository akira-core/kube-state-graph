...
- **PVC `volumename` + `svm` labels (Harvest join).** Each `type="pvc"` node's
  `labels` may additively carry `volumename` (the bound PersistentVolume name,
  from `kube_persistentvolumeclaim_info`) and `svm` (from the Harvest volume
  series matched on `volume_name` — the Trident custom-resource chain is
  removed). Both are **plain labels**. A key is set only when its value
  resolves non-empty, and `svm` is impossible without `volumename`.
  **`volumename` ≠ `volume`**. Harvest/kubelet legs are OPTIONAL
  (log-and-continue). Join misses while Harvest is present emit one aggregated
  `slog.Warn("netapp_volume_join_miss", "count", n)`. Tests:
  `pkg/build/netapp_test.go`,
