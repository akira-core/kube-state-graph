  ...
  **stock** Harvest `volume` label (the ONTAP FlexVol name). It is NOT a label
  ...
  - **hop A `volume_labels`** — the SOLE source of storage *topology*: the
    ...
    **A restricted read IS hub mode** (`topologyPlan.hub`, `claimscope.go`): the
    ...
    cluster identities stay per zone. Because the relaxed `ALERTS` read then
    holds every zone's alerts, alert matching AGREES ON ZONE (D11,
    `pkg/build/alerts.go`): an alert's `az`/`env` pair must agree with the
    ...
    excludes. It runs on every build path, not only hub mode. Tests: `pkg/build/volumelabelscope_test.go`
