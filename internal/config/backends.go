...
//
// Parsing lives here rather than in pkg/promql on purpose: pkg/ stays free of
// file I/O and of any parser dependency, and receives an already-validated
// promql.Table (design D3).
...
func ReadBackendsFile(path string, lookup LookupEnvFunc) (*promql.Table, error) {
	...
	return ParseBackendsFile(data, lookup)
}
...
func ParseBackendsFile(data []byte, lookup LookupEnvFunc) (*promql.Table, error) {
	...
	globalUser, _ := lookup("KSG_PROM_USERNAME")
	globalPass, _ := lookup("KSG_PROM_PASSWORD")
...
func SingleBackendTable(promURL, username, password string) (*promql.Table, error) {
	return promql.NewTable([]promql.Backend{
		promql.NewBackend(DefaultBackendName, promURL, promql.Families, nil, username, password),
	})
}
