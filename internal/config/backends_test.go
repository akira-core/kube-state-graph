...
func TestParseBackendsFile_YAMLAndJSONAreEquivalent(t *testing.T) {
	fromYAML, err := ParseBackendsFile([]byte(yamlTable), noEnv())
	require.NoError(t, err)
	fromJSON, err := ParseBackendsFile([]byte(jsonTable), noEnv())
	require.NoError(t, err)
...
func TestParseBackendsFile_ParsedShape(t *testing.T) {
...
// no signal at all.
func TestParseBackendsFile_RejectsUnknownField(t *testing.T) {
...
func TestParseBackendsFile_RejectsEmptyAndUnparseable(t *testing.T) {
	_, err := ParseBackendsFile([]byte("backends: []"), noEnv())
	require.Error(t, err)
	assert.Contains(t, err.Error(), "declares no backends")

	_, err = ParseBackendsFile([]byte("::: not yaml :::"), noEnv())
	require.Error(t, err)
}

func TestParseBackendsFile_RejectsUnknownFamily(t *testing.T) {
	_, err := ParseBackendsFile([]byte(`
backends:
  - name: zone-a
    url: http://vm-a:8428
...
func TestParseBackendsFile_DelegatesStructuralValidation(t *testing.T) {
	_, err := ParseBackendsFile([]byte(`
backends:
  - name: zone-a
    url: http://vm-a:8428
...
func TestParseBackendsFile_PerBackendCredentials(t *testing.T) {
	tbl, err := ParseBackendsFile([]byte(credTable), env(map[string]string{
...
func TestParseBackendsFile_GlobalPairIsTheFallback(t *testing.T) {
	tbl, err := ParseBackendsFile([]byte(yamlTable), env(map[string]string{
...
func TestParseBackendsFile_PerBackendOverridesGlobal(t *testing.T) {
	tbl, err := ParseBackendsFile([]byte(`
backends:
  - name: own
    url: http://vm-a:8428
    families: [ksm, kubelet, servicegraph, probe]
...
func TestParseBackendsFile_RejectsHalfDeclaredPair(t *testing.T) {
...
func TestParseBackendsFile_RejectsUnsetNamedVariable(t *testing.T) {
	for name, vars := range map[string]map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			_, err := ParseBackendsFile([]byte(credTable), env(vars))
			require.Error(t, err)
			assert.Contains(t, err.Error(), `backend "zone-a"`)
...
func TestParseBackendsFile_RejectsLiteralCredentials(t *testing.T) {
...
func TestParseBackendsFile_ErrorsNeverEchoCredentials(t *testing.T) {
...
func TestParseBackendsFile_DocumentedExampleParses(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("testdata", "backends-example.yaml"))
	require.NoError(t, err)

	tbl, err := ParseBackendsFile(data, env(map[string]string{
