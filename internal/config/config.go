...
type Config struct {
	...
	// Deployment / StatefulSet / DaemonSet / CronJob) scopes
	// (--netapp-qos-scope-batch-bytes / KSG_NETAPP_QOS_SCOPE_BATCH_BYTES) — so
	// a large estate is split across several queries instead of exceeding the
	// upstream's query-length limit.
...
func Parse(args []string, lookup LookupEnvFunc) (Config, error) {
	...
	fs.IntVar(&cfg.NetAppQoSScopeBatchBytes, "netapp-qos-scope-batch-bytes", cfg.NetAppQoSScopeBatchBytes, "Byte budget for one data-derived alternation: a scoped QoS query's `volume`, or a storage-graph pod/node/controller-name read's scope. A larger set is split across several queries.")
