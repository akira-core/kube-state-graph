...
摘要：一次 `/v1/graph` 會平行發出 **37** 條拓樸查詢（22 條 kube-state-metrics 遇 query 錯誤會失敗整次建圖；13 條 Harvest + 2 條 kubelet 為 log-and-continue），再發 **3** 條 service-graph 查詢（帶過濾且未載入任何 pod／service 時整組跳過），`up{}` 只在未過濾且拓樸為空時探測。**沒有 metric-name prefix**，一律用裸名查詢。v1 無結果快取。

K8s 形狀的系列預期帶有由 `vmagent`／Prometheus `external_labels` 寫入的 `cluster` 標籤。Harvest 的 `cluster` 是 **ONTAP** 叢集，不用來當 `?cluster=`。

下表「必填？」指的是**空向量**會不會丟掉該功能。kube-state-metrics 22 條或 `traces_service_graph_request_total` 的 **query 錯誤**（逾時／5xx）會失敗整次建圖；Harvest、kubelet 與兩條 RED 系列則 log-and-continue。細節見目錄。
