...
摘要：一次 `/v1/graph` 會平行發出 **37** 條拓樸查詢（19 條 kube-state-metrics 遇 query 錯誤會失敗整次建圖；3 條為 log-and-continue——2 條基數隨歷史累積的 annotation 族，以及基數隨容器、image 變體與 pod 汰換倍增的 `kube_pod_container_info`；13 條 Harvest + 2 條 kubelet 為 log-and-continue），再發 **3** 條 service-graph 查詢（帶過濾且未載入任何 pod／service 時整組跳過），`up{}` 只在未過濾且拓樸為空時探測。**沒有 metric-name prefix**，一律用裸名查詢。v1 無結果快取。`/v1/storage-graph` 讀得更少：不發 `kube_pod_container_info` 與四條 Service／EndpointSlice 查詢（body 不帶 `data.containers`、也沒有 service 節點），並改依參照讀 pod、Kubernetes node 與 controller：`kube_pod_info`／`kube_pod_owner` 只讀 claim binding、`pod=` root，或（請求以 `application=<argo-app>` 為 root 時）該 Application 的 recovery 讀回的 pod——即使這些 pod 沒有掛載任何 claim 也會載入。`application=` 是 workload 側 root：路徑上的 pod 或 claim 帶有該 Application 就保留，而且會抑制「只有 pod root 才推導 namespace」。其餘依參照讀取不變：四條 `kube_node_*` 只讀這些 pod 排程所在的節點或 `node=` root 點名的節點；八條 controller-owner／controller-annotation 只讀這些 pod 解析出的 owner 名稱。以 `ontap_cluster=`、`aggr=` 或 `svm=` 為 root 的請求改走 **volume hub**：claim 系列改從被指名元件的 `volume_labels` rows 反查（FlexVol 名稱內嵌的 `pvc_<uid>` 即 PV `pvc-<uid>`），且所有 Kubernetes 查詢都不帶 `az`／`env` matcher，因此跨 zone 或 environment 共用的 filer 會畫出其上所有 claim（`az` 仍負責選 Harvest store）。靜態佈建的 PV 無法從 storage root 找到，詳見 [`docs/netapp-harvest-preconditions.md`](docs/netapp-harvest-preconditions.md)。
...
| `kube_pod_container_info` | Pod 容器清單 `data.containers` = `[{name, image}]`，依 `(name, image)` 排序；視窗內換 image 時每容器取最新觀測到的 image | `cluster`, `namespace`, `pod`, `container`, `image` | 選填（缺或 query 錯誤則無 `data.containers`）；屬 KSM 預設。`/v1/storage-graph` 不讀 |
...
| 指標 | 用途 | 會讀的標籤 | 必填？ |
|---|---|---|---|
| `volume_labels` | **Hop A — 整個儲存拓樸。** PVC→aggregate join（由 PV 名推導出的 token 比對 `volume`）、`netapp-aggr`／`netapp-node` 實體、PVC `svm` 標籤。info series：只讀 labels、丟棄 sample 值。不帶任何 request matcher；唯一例外是以 `ontap_cluster=`／`aggr=`／`svm=` 為 root 的 `/v1/storage-graph`，會改為只讀被指名的元件（`svm=` 另把該 SVM 碰到的每個 aggregate 整個重讀一次，以維持 owner vote），並由這些 rows 反查 claim | `cluster`（ONTAP 叢集）、`node`、`aggr`、`svm`、`volume` | 選填（缺則無 NetApp 節點／邊／`svm`，且完全不發出 QoS 查詢） |
