...
- [x] 1.1 Read `client_network_peer_address` off the series in `parseServiceGraph`, next to
...
- [x] 1.2 Widen `resolveServer` and `resolveUnknownServerPeer` by one `string` parameter for
...
- [x] 1.3 Reorder the first-non-empty peer-value pick in `resolveUnknownServerPeer` to
...
- [x] 1.4 Add a pure bracket-truncation helper — cut the value at the **first `[` whose
...
- [x] 1.5 Confirm the external fallback still passes the RAW, wholly unnormalised label
...
- [x] 1.5b Update the `stripPeerAddressPort` doc comment
...
- [x] 1.6 Confirm no stage below the pick branches on which label supplied the value
...
- [x] 2.1 `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressResolvesService` —
...
- [x] 2.2 `TestParseServiceGraph_UnknownServerPeerLabel_ServerAddressWinsPrecedence` —
...
- [x] 2.2b `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressBeatsNetPeerName`
...
- [x] 2.2c `TestParseServiceGraph_UnknownServerPeerLabel_NoFallThroughOnNonClassifying` —
...
- [x] 2.3 **(explicitly required)** `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressBracketSuffixResolves`
...
- [x] 2.4 **(explicitly required)** `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressBracketSuffixExternalKeepsRaw`
...
- [x] 2.5 `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressBracketDistinctExternals`
...
- [x] 2.6 `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerAddressBracketIPLiteral` —
...
- [x] 2.7 `TestParseServiceGraph_UnknownServerPeerLabel_BracketSuffixOnNetPeerName` —
...
- [x] 2.8 `TestParseServiceGraph_UnknownServerPeerLabel_NetworkPeerPortIgnored` — a series
...
- [x] 2.9 Verify the existing `TestParseServiceGraph_UnknownServerPeerLabel_*` cases still
...
- [x] 2.9b Add an IPv6 topology fixture for 2.10 / 2.11: `sampleTopologyWithServices()`
...
- [x] 2.10 `TestParseServiceGraph_UnknownServerPeerLabel_BracketedIPv6NotTruncated` — the
...
- [x] 2.11 `TestParseServiceGraph_UnknownServerPeerLabel_BracketedIPv6WithIdentifierExternal`
...
- [x] 2.12 `TestParseServiceGraph_UnknownServerPeerLabel_TruncationPromotesToBareShortName`
...
- [x] 3.1 Add one fixture-driven case to `internal/integration/graph_e2e_test.go`, alongside
...
- [x] 4.1 Update the "Unknown-server peer-label enrichment" bullet in `CLAUDE.md`: three
...
- [x] 5.1 `go test ./pkg/build/... -run TestParseServiceGraph -v` green.
- [x] 5.2 `go vet ./...` clean; `make lint` reports no NEW issues (confirm any finding
      predates this change via `git stash`).
- [x] 5.3 Full `make test` (race + shuffle) green locally. Docker was available, so the
      integration suite ran for real (598s, ok) rather than skipping. One unrelated flake
      surfaced (`internal/api` `TestTracing_EdgeTypesEmitsServerSpan`, an async-span-flush
      timing test with no code touched by this change) — reran clean 3x isolated and clean
      as part of a full `internal/api` -race -shuffle rerun; not caused by this change.
- [x] 5.4 Integration subset from 3.1 green against a real VictoriaMetrics container, or
      explicitly deferred to CI with the reason stated.
- [x] 5.5 `openspec validate "resolve-unknown-server-network-peer-address"` passes.
