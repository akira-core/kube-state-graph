...
- [x] 1.1 Split `serviceGraphSentinelSelector` into an independent client matcher
...
- [x] 1.2 `pkg/promql/queries_test.go` (or equivalent): assert the rendered selector string
      for `QServiceGraphTotal` reflects the split matchers.

## 2. Resolver (`pkg/build/servicegraph.go`)

- [x] 2.1 In `resolveServer`, before falling through to `resolveEmptyUID` (empty-UID path)
...
- [x] 2.2 Add `resolveUnknownServerPeer`: takes whether the client side resolved to a real
      ...
      present.
- [x] 2.3 Add the peer-address classification helper: strip an optional `:<port>` suffix
...
- [x] 2.4 Wire a successful classification into the existing `resolveServiceLevel(anchorCluster,
...
- [x] 2.5 Extend `sgTrace` / debug logging as needed so a peer-label enrichment hit/miss is
...
- [x] 3.1 `TestParseServiceGraph_UnknownServerPeerLabel_NetPeerNameResolvesService` — client
...
- [x] 3.2 `TestParseServiceGraph_UnknownServerPeerLabel_ServerAddressFallback` — `client_net_peer_name`
      absent, `client_server_address` (with a port suffix) used instead; same resolution as 3.1.
- [x] 3.3 `TestParseServiceGraph_UnknownServerPeerLabel_BareShortName` — bare single-label
      value resolves within the client pod's own namespace.
- [x] 3.4 `TestParseServiceGraph_UnknownServerPeerLabel_ExternalAddress` — multi-label
      non-`.svc` value → `external/<value>` node, `pod-calls-pod` edge.
- [x] 3.5 `TestParseServiceGraph_UnknownServerPeerLabel_AnchorLacksService` — classifies to a
      valid `(ns, svc)` but the anchor cluster doesn't hold it → external, not dropped.
- [x] 3.6 `TestParseServiceGraph_UnknownServerPeerLabel_NeitherLabelPresent_Dropped` — both
      labels empty/absent → no node, no edge.
- [x] 3.7 `TestParseServiceGraph_UnknownServerPeerLabel_ClientUnresolved_Dropped` — client
      side does not resolve to a real pod, peer label present → still dropped (enrichment
      does not apply).
- [x] 3.8 `TestParseServiceGraph_UnknownServerPeerLabel_ServerUIDPresentButUnresolved` —
...
- [x] 3.9 Regression: an existing test asserting the old combined `server!~"user|unknown"`
      selector string is updated for the split matchers.

## 4. Integration (`internal/integration`)

- [x] 4.1 One fixture-driven test against the real VictoriaMetrics testcontainer: ingest a
...
- [x] 5.1 Update `CLAUDE.md`'s D30 bullet: narrow the server-side sentinel scope and add a
      pointer to the new peer-label enrichment rule (state the "every other case still
      drops" invariant explicitly).

## 6. Verify

- [x] 6.1 `go test ./pkg/build/... ./pkg/promql/... -run TestParseServiceGraph -v` and
