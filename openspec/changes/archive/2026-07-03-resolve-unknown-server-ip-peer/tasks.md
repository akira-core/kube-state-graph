...
- [x] 1.1 Add `ipKey{cluster, ip string}` type.
- [x] 1.2 In `parseServiceGraph`, alongside the existing `svcCandidates` build, build
...
- [x] 1.3 Thread `ipIndex` onto `sgResolver`.

## 2. Classification (`pkg/build/servicegraph.go`)

- [x] 2.1 In `resolveUnknownServerPeer`, after the existing `classifyBareShortName` miss and
      ...
      svc)` call unchanged.
- [x] 2.2 A miss (not an IP literal, or an IP literal absent from `ipIndex` for
      `anchorCluster`) falls to the existing `external(rawValue)` path unchanged.
- [x] 2.3 Add a distinct `noteExternal` reason string for the IP-literal-miss case
...
- [x] 3.1 `TestParseServiceGraph_UnknownServerPeerLabel_IPLiteralResolvesService` — bare IP in
...
- [x] 3.2 `TestParseServiceGraph_UnknownServerPeerLabel_IPLiteralWithPort` — `IP:port` value
      strips the port before IP-literal matching, same resolution as 3.1.
- [x] 3.3 `TestParseServiceGraph_UnknownServerPeerLabel_IPLiteralFamilySiblingNotMatched` — IP
...
- [x] 3.4 `TestParseServiceGraph_UnknownServerPeerLabel_IPLiteralNoMatch` — IP absent from the
      anchor cluster's `ClusterIP` set entirely → `external/<ip>`.
- [x] 3.5 `TestParseServiceGraph_UnknownServerPeerLabel_IPLiteralDuplicateClusterIP` —
...
- [x] 3.6 Regression: confirm existing `classifyBareShortName`/`classifyK8sDNS` cases from
      `resolve-unknown-server-peer-labels` are unaffected (IP branch never fires for a
      non-IP host). Full `TestParseServiceGraph*` suite green (0.017s, all pre-existing +
      new cases pass).

## 4. Integration (`internal/integration`)

- [x] 4.1 One fixture-driven case against the real VictoriaMetrics testcontainer: a
      ...
      `TestUnknownServerIPPeerResolvesToServiceNode` — PASS (20.40s) against real VM.

## 5. Docs

- [x] 5.1 Update `CLAUDE.md`'s "Missing pod-UID human-label fallback" / unknown-server bullet
...
- [x] 6.1 `go test ./pkg/build/... -run TestParseServiceGraph -v` green.
- [x] 6.2 `go vet ./...` clean; `make lint` 0 new issues (pre-existing, unrelated
      `exhaustive` finding in `pkg/graph/project.go` confirmed via `git stash` to predate
      this change).
- [x] 6.3 Affected integration subset green against a real VM (the new fixture from 4.1, plus
      existing unknown-server-peer-label integration tests for regressions).
      `TestUnknownServerIPPeerResolvesToServiceNode`, `TestUnknownServerPeerLabelResolvesToServiceNode`,
      `TestSentinelPeersExcludedAtQueryLayer` — 3/3 PASS (68.81s total).
- [x] 6.4 Full `make test` (race + shuffle) green locally — all packages, including the full
      Docker-gated `internal/integration` suite (532.4s total).
- [x] 6.5 `openspec validate "resolve-unknown-server-ip-peer"` passes.
