...
- [x] 2.2 Add ten constants + `last_over_time` render cases (no `rate()`, no `sum by`): `QVolumeReadOps`, `QVolumeWriteOps`, `QVolumeReadLatency`, `QVolumeWriteLatency`, `QAggrStatus`, `QAggrSpaceUsed`, `QAggrSpaceTotal`, `QNetAppNodeStatus`, `QKubeletVolumeUsedBytes`, `QKubeletVolumeCapacityBytes`; unit-test each rendered string.
...
- [x] 3.3 Add `graph.IOMetrics{ReadOps, WriteOps, ReadLatencyUs, WriteLatencyUs *float64}` and `Edge.IO *IOMetrics` (with a `WithIO` copy helper mirroring `WithMetrics`); RED `EdgeMetrics` untouched.
...
- [x] 4.2 Add the ten new legs to `ReadTopology` with **log-and-continue** error semantics (empty vector on leg failure, existing KSM legs keep abort semantics); pin with a unit test that a failing Harvest leg does not fail the build.
...
- [x] 6.1 Metrics union DTO: `Rate` → `*float64 omitempty`, add four `omitempty` I/O fields, `metricsDTO(m, io)` fills exactly one family (RED precedence on the impossible both-set case), `round6` on every field; unit tests RED-only / IO-only / neither.
