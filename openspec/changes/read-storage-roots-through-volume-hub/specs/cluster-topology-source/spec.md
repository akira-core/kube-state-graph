...
- `az` and `env`: every kube-state-metrics and kubelet series — except in a hub-mode `/v1/storage-graph` build (the `storage-graph-api` capability's "Storage-side roots read the claim chain through the volume hub"), which renders neither on any query. **Never** a NetApp Harvest series — the `az` dimension reaches Harvest through backend selection only (the `upstream-backend-routing` capability's zone rule), and `env` does not reach Harvest at all.
...
#### Scenario: A hub-mode storage build renders no zone or environment matcher

- **WHEN** a `/v1/storage-graph` build for `?az=zone-a&env=prod&namespace=shop&aggr=aggr00` runs in hub mode
- **THEN** every kube-state-metrics, kubelet and `ALERTS` query carries `namespace` (in its or-absent form on `ALERTS`) and no `<az-key>` or `<env-key>` matcher

### Requirement: Backend routing composes with request-scoped selectors

Backend selection and PromQL matcher rendering SHALL be independent, composed mechanisms. The `az` dimension SHALL continue to be rendered as a label matcher on every query that accepts it — exactly as specified by "Request-scoped upstream selectors" — **in addition to** selecting which backends the query is issued to. Neither mechanism SHALL substitute for the other: routing narrows which store is asked, the matcher narrows what that store returns. The `harvest` family is the one family where the two diverge: it is zone-routed yet accepts no `az` matcher, so backend selection is the only effect `az` has on it (see the `netapp-storage-graph` capability). A hub-mode `/v1/storage-graph` build (the `storage-graph-api` capability's "Storage-side roots read the claim chain through the volume hub") is the one BUILD where they diverge: `az` still selects the `harvest` backends, while the `ksm`, `kubelet` and `alerts` families receive neither the `az` matcher nor `az`-based backend selection — both decisions taken from one routing snapshot.
...
#### Scenario: Hub mode routes Harvest by zone and the Kubernetes families everywhere

- **WHEN** a hub-mode `/v1/storage-graph` build runs with `az=zone-a` against backends declaring `zones: [zone-a]` and `zones: [zone-b]` for every family
- **THEN** every Harvest query is issued to the `zone-a` backend only, and every `ksm`, `kubelet` and `alerts` query is issued to both backends with byte-identical query strings carrying no `az` matcher
