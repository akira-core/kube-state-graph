...
- **WHEN** a hub-mode build runs with `az={zone-a}`, `env={prod}`, a `zone-a` `harvest` backend returns a rooted FlexVol whose claim lives in an `env="dev"` cluster, and no `cluster` or `namespace` value is given
- **THEN** the claim is loaded, its derived token is matched against the rooted FlexVol by the configured forward derivation, and the claim receives its aggregate and SVM exactly as an unfiltered build would give them
