...
- [ ] 10.3 Bring up the demo repository with `make up`, issue a rooted and an unrooted `/v1/storage-graph` request for the same window, and confirm the bodies agree on every node and edge the rooted request retains and that `make verify` stays green
