...
- [ ] 1.1 `deploy/docker/server.Dockerfile`: declare `ARG ENVOY_TOOLS_IMAGE=<digest-pinned>`
...
- [ ] 1.2 `Makefile`: derive `ENVOY_TOOLS_IMAGE` from the Dockerfile declaration (single
      ...
      (no container run).
- [ ] 1.3 `internal/integration/route_e2e_test.go` `RouteSuite.SetupSuite`: `Fatalf` when
      `os.Getenv("CI") != ""`, `Skipf` otherwise. Correct the file comment that currently
      claims CI runs the suite.
- [ ] 1.4 `.github/workflows/ci.yml` `test` job: run `make router-check-tool`, export
      `KSG_ROUTER_CHECK_BIN` via `$GITHUB_ENV`, and change the test step to `make test`.
- [ ] 1.5 `.github/workflows/ci.yml`: add an independent `route-containment` job running
      `make check-route-containment`.
- [ ] 1.6 Push and verify on a real runner (see the verification checklist below) — this
      group is unverifiable locally.

## 2. Multi-IP snapshot integrity (RED → GREEN)

- [ ] 2.1 `pkg/route/snapshot/snapshot_test.go`: duplicated VirtualService rows in the
      snapshot → `ScopedFor` returns one config per `(kind, ns, name)`; duplicated backend
      Service rows → one registry entry per hostname.
- [ ] 2.2 `pkg/route/resolver_test.go`: two destination IPs whose `LoadTrafficAt` calls
...
- [ ] 2.3 `pkg/route/resolver.go` `loadSnapshot`: dedup the union by
      `(Cluster, Namespace, Name, ValidFrom)` per row kind.
- [ ] 2.4 `pkg/route/snapshot/snapshot.go`: `ScopedFor` dedups `Configs` by
      `(GroupVersionKind, Namespace, Name)`; `backendServices` dedups by hostname.

## 3. Backend destination host resolution (RED → GREEN)

- [ ] 3.1 `pkg/route/translate/translate_test.go`: a bare short `destination.host` →
      the emitted Envoy cluster names the full in-cluster FQDN (today: `<name>.<ns>`).
- [ ] 3.2 `pkg/route/store/store_test.go`: `ParseBackendHost` accepts exactly
      `<name>.<ns>.svc.cluster.local`; rejects 3+ leading labels and every relative form.
- [ ] 3.3 `pkg/route/store/store.go`: add the cluster-domain constant, derive
      `serviceDomain` from it, tighten `ParseBackendHost` to exactly two labels.
- [ ] 3.4 `pkg/route/snapshot/snapshot.go` `ScopedFor`: stamp `Domain` on the Gateway and
      VirtualService `config.Meta`.
- [ ] 3.5 Normalise destination hosts at collection time using the owning VirtualService's
...
- [ ] 4.1 `pkg/route/snapshot/snapshot_test.go`: same-named live Gateways in two
      namespaces → `ScopedFor` selects the requested namespace's row and only its bound
      VirtualServices.
- [ ] 4.2 `pkg/route/snapshot/snapshot_test.go`: two live ingress Service identities on one
...
- [ ] 4.3 `pkg/route/snapshot/snapshot.go`: `ScopedFor(ns, name)`; Hop 1 collects and
      degrades on multiple identities; Hop 2 unions pod labels.
- [ ] 4.4 `pkg/route/resolver.go`: carry the candidate's namespace through
...
- [ ] 5.1 `pkg/route/resolver.go`: the three error returns in `resolve` return the empty
      outcome; document that the outcome is meaningless when the error is non-nil.
- [ ] 5.2 `pkg/route/matchcheck/matchcheck.go`: `parseActuals` errors unless it recovered
      one result per query; test with a stray numeric line in a multi-query parse.
- [ ] 5.3 `pkg/route/store/clickhouse.go`: warn once (via `sync.Once`) the first time a
      collapse occurs under `uniqueRows`, naming the writer-uniqueness violation and the
      flag.

## 6. Performance (RED → GREEN)

- [ ] 6.1 `pkg/build/routeprescan_test.go`: the index from concurrent resolution equals the
      serial index; exceeding the key cap truncates and logs the dropped count.
- [ ] 6.2 `pkg/build/routeprescan.go` `resolveRouteQueries`: bounded `errgroup`, mutex
      around the index write, key cap with logged truncation.
- [ ] 6.3 `pkg/route/scoped.go`: mutex-guard the probe memo; update the doc comment that
      currently states the scope is serial and needs no locking.
- [ ] 6.4 `pkg/build/servicegraph.go` `ReadServiceGraph`: build the `sgResolver` once and
...
- [ ] 7.1 `pkg/graph/registry.go`: correct the `service-selects-pod` and `pod-calls-pod`
...
- [ ] 7.2 `internal/api/handlers.go`: correct the `/v1/graph` `@Description` (node types,
...
- [ ] 7.3 Comment drift sweep over `pkg/route` and `pkg/build/route*.go` for
...
- [ ] 7.4 Re-read `CLAUDE.md`'s route-resolution section against the behaviour changed
...
- [ ] 8.1 `internal/integration/route_e2e_test.go`: dual-stack `client_dns_answers` (two
      IPs on one ingress Service) resolves to the routed backend Service.
- [ ] 8.2 `internal/integration/route_e2e_test.go`: a VirtualService using a bare short
      `destination.host` resolves to the backend Service.

## 9. Verification

- [ ] 9.1 `go test ./pkg/route/... ./pkg/build/ -count=1 -race`
- [ ] 9.2 `KSG_ROUTER_CHECK_BIN=... CI=1 go test ./internal/integration/ -run 'TestRouteSuite|TestRouteStoreSuite' -v`
- [ ] 9.3 `make ci` green (lint + vuln + test + docs + mocks + containment)
