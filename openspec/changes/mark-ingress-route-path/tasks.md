...
- [x] 1.1 `pkg/graph/edge.go`: add `EdgeTypePodRoutesToService EdgeType = "pod-routes-to-service"`
      to the const block.
- [x] 1.2 `pkg/graph/registry.go`: add the `EdgeTypeDefinition` — `SourceType: []NodeType{NodeTypePod}`
...
- [x] 1.3 `pkg/graph/project.go`: add `EdgeTypePodRoutesToService` to the connectivity case of
      `connectivityExcluded`'s switch (the switch is documented as exhaustive over `EdgeType`).
- [x] 1.4 `pkg/graph/service_test.go` (or a new `registry_test.go` case): assert the type is
...
- [x] 1.5 `pkg/graph/project_prune_test.go`: a pod connected ONLY by a `pod-routes-to-service`
...
- [x] 4.1 Update the existing chain assertions: the synthesized `igw0 → payments` edge moves out of
...
- [x] 4.2 New: chained hit ⇒ ingress node's `Labels()["role"] == "ingress-gateway"`; backend
      service node has NO `role` key.
- [x] 4.3 New: `RouteIngressLBService` fallback ⇒ node's `role == "ingress-lb"` and zero
      `pod-routes-to-service` edges.
- [x] 4.4 New (determinism): a fixture where one endpoint chains through Service X and another
      LB-falls-back to the same X ⇒ `role == "ingress-gateway"` for BOTH input orderings (assert
      by feeding the two samples in each order).
- [x] 4.5 Existing degrade tests gain: no `role` key on any service node, zero
      `pod-routes-to-service` edges, node/edge set byte-identical to the pre-change direct shape.
- [x] 4.6 Engine-off / no-resolver parse tests: assert no `role` key and no new-type edge anywhere
      (feature-off output unchanged).

## 5. API surface

- [x] 5.1 `internal/api/handlers.go`: extend the `/v1/graph` and `/v1/edge-types` swagger
...
- [x] 5.3 `go test ./internal/api/ -run Golden -update` — regenerate
...
- [x] 6.2 nginx LB-fallback case: assert `labels.role == "ingress-lb"` and no
      `pod-routes-to-service` edge in the body.
- [x] 6.3 Chain-degrade case: assert no `role` key and no `pod-routes-to-service` edge.

## 7. Documentation

- [x] 7.1 `CLAUDE.md`: update the route-resolution bullet's chain description (new edge type +
...
- [x] 7.2 `docs/istio-virtualservice-routing-history-design.md`: §4.1 / §4.2 diagrams and tables,
...
- [x] 8.2 `make test` (full, `-race -shuffle=on` — an order-dependent marker would fail here)
- [x] 8.3 `make check-route-containment` (`pkg/build` still must not import `pkg/route`)
- [x] 8.4 `make verify-mocks` (no interface changed — must be a no-op)
- [x] 8.5 Docker + `router_check_tool`:
      `KSG_ROUTER_CHECK_BIN=$(which router_check_tool) go test ./internal/integration/ -run TestRouteSuite`
- [x] 8.6 `openspec verify "mark-ingress-route-path"` before archiving.
