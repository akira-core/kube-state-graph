...
- [x] 9.5 Run `make test`, `make vet`, `make lint` and `make check-docs`; verify all pass.

## 11. Volume granularity moved into the reader

- [x] 11.1 Delete `promql.qosVolumeGranularitySelector` and its two uses (the `Render` switch case for the six workload queries and the `RenderQoSVolumeScoped` matcher list), so an issued QoS query carries the `volume` scope and nothing else; verify `pkg/promql` tests and refresh `pkg/promql/testdata/render-baseline.txt` (six lines lose `{lun=""}`).
- [x] 11.2 Add a `lun` field to `qosCandidate`, read it in `indexQoSFamily`, and skip non-empty-`lun` candidates in `sumQoSIO`; verify with a `pkg/build` test asserting a volume row of 150 plus a LUN row of 90 still reports `read_ops: 150`, and one asserting a LUN-only FlexVol yields an edge with no `metrics` at all.
- [x] 11.3 Let `pickPolicy` read every in-scope candidate and prefer a `policy_group` the fixed-policy index holds for `(oc, svm)`, falling back to the lexically-smallest non-empty value; verify with a test pinning the `ontap-san` shape (volume row in `User-Best_effort`, LUN row in `gold-tier`) and one asserting the built-in class does not win despite sorting first.
- [x] 11.4 Move the `internal/integration` fixture to the SAN shape — volume row in `User-Best_effort`, LUN row carrying the real policy — and assert `read_ops: 150` alongside the resolved ceiling; verify the integration suite passes with Docker available.
- [x] 11.5 Update `docs/upstream-metrics.md`, `docs/netapp-harvest-preconditions.md`, both READMEs, `docs/BREAKING.md` and the `CLAUDE.md` storage bullet so no document still describes `{lun=""}` as a query-layer contract; verify `make test`, `make vet`, `make lint` and `make check-docs`. **`make test` / `make vet` / `make lint` pass (lint: 0 issues; gofmt clean). Golden `with-netapp-storage-cytoscape.json` is unchanged (hand-built View, not hop-C). `make check-docs` exits non-zero because it diffs the WHOLE `docs/` tree and this change edits `docs/*.md` by hand — the generated artefacts it regenerates, `docs/swagger.{json,yaml}`, are verified in sync (`git diff --quiet -- docs/swagger.json docs/swagger.yaml`). It goes green once the docs are committed.**
