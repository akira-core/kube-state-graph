...
// sample that feed the unknown-server enrichment: the three D1-precedence-
// ordered peer-address labels (resolve-unknown-server-peer-labels,
// resolve-unknown-server-network-peer-address), plus the two OPTIONAL
// dimensions added by translate-global-fqdn-to-k8s-service (destination IPs
// and listener port).
type peerLabels struct {
	serverAddress      string // client_server_address (checked first — D1)
	networkPeerAddress string // client_network_peer_address (checked second — D1)
	netPeerName        string // client_net_peer_name (checked third — deprecated)
...
// value returns the peer-address value under the D1 precedence: the first
// non-empty of client_server_address, client_network_peer_address,
// client_net_peer_name wins outright — never merged with, nor falling back
// from, a lower-precedence label that later fails to classify. "" when all
// three are absent.
...
// that byte and everything after it. Runs BEFORE net.SplitHostPort so it sees
// a well-formed authority.
//
...
// value, after truncateBracketSuffix has removed any bracketed connection
...
// yields the truncated, unsplit value as the host and "" as the port. A
// bracketed IPv6 authority ([<ipv6>]:<port>) is the one bracketed shape
// net.SplitHostPort handles correctly on its own — it strips the brackets and
// the port together — which is why truncateBracketSuffix guards index 0
// rather than cutting unconditionally. This is the pre-change
...
func splitPeerAddressPort(value string) (host, port string) {
	truncated := truncateBracketSuffix(value)
	h, p, err := net.SplitHostPort(truncated)
	if err != nil {
		return truncated, ""
	}
	return h, p
