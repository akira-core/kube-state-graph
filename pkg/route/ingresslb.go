...
// ingressIdentityStatus classifies the as-of identity dedup of the
...
// snapshot. It is the identity-dedup core shared by the ingress LB Service
...
// LoadTrafficAt — no store read.
//
// Per IP the candidate identities are the distinct (namespace, name) of every
// service row carrying the IP that is LIVE at the resolution instant
// (simplify-route-resolution-to-point-in-time D5: version churn of one
// identity still counts once, and an identity superseded earlier does not
// count at all — only simultaneous owners collide). Merge precedence over IPs
// is order-free (sibling design D7):
...
func ingressServiceIdentity(snap *snapshot.Snapshot, ips []string) (ingressIdentity, ingressIdentityStatus) {
	var (
	...
	for _, ip := range ips {
		...
		for _, r := range snap.ResolveIPToIngressServices(ip) {
...
// pipeline produced no hit over the loaded (selected-cluster) snapshot — the
// caller additionally gates on the miss being RouteNoGateway, so a deeper
// Istio miss keeps its diagnostic reason — does the destination IP set map to
// exactly ONE ingress LB Service identity at the resolution instant?
...
func resolveIngressLBService(snap *snapshot.Snapshot, ips []string) (build.RouteDestination, build.RouteOutcome, bool) {
	id, status := ingressServiceIdentity(snap, ips)
	switch status {
