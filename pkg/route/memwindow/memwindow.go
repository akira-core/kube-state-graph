...
// the versions overlapping [t0,t1) loaded once by the store, it splits the
// interval at every version boundary (Segments) and answers the same 3-hop
// IP->Gateway and per-gateway ScopedFor a point-in-time store query would — but
// against the already-loaded rows, with no further DB round-trips. Ported
// verbatim from poc/route2a/internal/memwindow.
//
// The set/AsOf logic mirrors the ClickHouse reader exactly: has -> Contains, hasAll ->
...
func (m *Window) ConfigSigAt(gwName string, t time.Time) (uint64, bool) {
	...
	h := fnv.New64a()
	_, _ = fmt.Fprintf(h, "g:%s/%s\x00%s\x01", gw.Namespace, gw.Name, gw.SpecJSON)
